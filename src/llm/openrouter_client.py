...
from src.utils.config import settings as default_settings
...
class OpenRouterClient:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info("Initializing OpenRouter with:")
            logger.info(f"  Base URL: {base_url}")
            logger.info(f"  Model: {model}")
            logger.info("  API Key: %s", "Set" if self.settings.openrouter_api_key else "Not set")
    ...
    def generate(
        ...
        try:
            ...
            if "404" in error_str:
                ...
                logger.error(
                    f"Model may not be available. Check OPENROUTER_MODEL: {self.model_name}"
                )
            else:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            messages = [
                SystemMessage(
                    content="You are a test assistant. Respond with 'OK' if you can hear me."
                ),
