...
def init_services() -> tuple:
    ...
    try:
        ...
        github_client = GitHubClient(
            token=settings.github_token, repo_name=settings.github_repository
        )
...
@click.option(
    "--mode",
    type=click.Choice(["daemon", "single"]),
    default="daemon",
...
def run(mode: str, issue_number: Optional[int], pr_number: Optional[int]):
    ...
    try:
        ...
        console.print(
            f"[green]✓ GitHub connection successful: {github_client.repo.full_name}[/green]"
        )

        if mode == "single":
            if issue_number:
                process_single_issue(github_client, issue_number, settings)
            elif pr_number:
                process_single_pr(github_client, pr_number, settings)
            else:
                click.echo("Please specify --issue-number or --pr-number for single mode")
        else:
            run_daemon(github_client, settings)
...
def process_single_issue(github_client: GitHubClient, issue_number: int, settings: Settings):
    ...
    try:
        ...
        code_agent = CodeAgent(github_client, settings)
        pr_number = code_agent.process_issue(issue_number)

        if pr_number:
            console.print(
                f"[green]✓ Successfully created PR #{pr_number} for issue #{issue_number}[/green]"
            )
        else:
...
def process_single_pr(github_client: GitHubClient, pr_number: int, settings: Settings):
    ...
    try:
        ...
        reviewer_agent = ReviewerAgent(github_client, settings)
        reviewer_agent.review_pull_request(pr_number)
...
def run_daemon(github_client: GitHubClient, settings: Settings):
    ...
    processor = IssueProcessor(github_client, settings)
    check_interval = settings.daemon_interval_seconds
...
def test_openrouter():
    ...
    try:
        ...
        if llm_client.test_connection():
            ...
            console.print("[green]✓ Text Generation: OK[/green]")
